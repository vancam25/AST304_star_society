...
# take the absolute value of the returned arrays to ensure parameter space is within
    # the correct bounds
delta_m_sample_1, delta_m_sample_2 = np.abs(af.sample_param_normal(num_samples, delta_m, std_delta_m))

# initialize arrays of shape ['num_samples', 4]
delta_m_sample_1_m_r_p = np.zeros((num_samples,4))
delta_m_sample_2_m_r_p = np.zeros((num_samples,4))

# run integrations to get integration results for all randomly generated parameters
for i in range(num_samples):
    delta_m_sample_1_m_r_p[i] = af.test_param_routine(Pc_guess, delta_m, delta_m_sample_1[i], xi)
    delta_m_sample_2_m_r_p[i] = af.test_param_routine(Pc_guess, delta_m, delta_m_sample_2[i], xi)

# save integration results to csv files
param_analysis_dir = ''

pd.DataFrame(delta_m_sample_1).to_csv(param_analysis_dir+'delta_m_sample_1_indep.csv')
pd.DataFrame(delta_m_sample_2).to_csv(param_analysis_dir+'delta_m_sample_2_indep.csv')

pd.DataFrame(delta_m_sample_1_m_r_p).to_csv(param_analysis_dir+'delta_m_sample_1.csv')
pd.DataFrame(delta_m_sample_2_m_r_p).to_csv(param_analysis_dir+'delta_m_sample_2.csv')